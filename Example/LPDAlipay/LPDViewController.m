...
@property(nonatomic, strong)LPDViewController *webvc;
...
- (void)doAPPay
{
    ...
    LPDOrderInfo* order = [LPDOrderInfo new];
    ...
    // NOTE: 商品数据
    order.biz_content = [LPDBizContent new];
    ...
    LPDRSASigner* signer = [[LPDRSASigner alloc] initWithPrivateKey:((rsa2PrivateKey.length > 1)?rsa2PrivateKey:rsaPrivateKey)];
...
- (void)doAPAuth
{
    ...
    //生成 auth info 对象
    LPDAuthInfo *authInfo = [LPDAuthInfo new];
    authInfo.pid = pid;
